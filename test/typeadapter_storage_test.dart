  ...
  group('TypeAdapter Storage Tests', () {
    ...
    group('Conversation TypeAdapter Tests', () {
      ...
      test('should handle large message lists efficiently', () async {
        final largeMessages = List.generate(1000, (index) => {
          'role': index % 2 == 0 ? 'user' : 'assistant',
          'content': 'Message content $index with some longer text to test serialization performance',
        ...
        print('Save time: ${saveTime}ms, Retrieve time: ${retrieveTime}ms');
    ...
    group('ChatProfile TypeAdapter Tests', () {
      test('should serialize and deserialize ChatProfile correctly', () async {
        final profile = ChatProfile(
          ...
          activeModelTools: [
            ModelTool(modelId: 'model1', providerId: 'provider1', toolName: 'search'),
          ],
        );
        ...
        expect(retrieved.config.systemPrompt, equals('You are a helpful assistant'));
    ...
    group('Performance Comparison Tests', () {
      test('should demonstrate TypeAdapter performance benefits', () async {
        final profiles = List.generate(100, (index) => ChatProfile(
          id: const Uuid().v4(),
          name: 'Profile $index',
          config: LlmChatConfig(
          ...
          activeMcp: [
            ActiveMcp(id: 'mcp$index', activeToolNames: ['tool1', 'tool2', 'tool3']),
          ],
        ));
        ...
        print('Bulk save 100 profiles: ${saveTime}ms');
        print('Bulk read 100 profiles: ${readTime}ms');
        print('Average save per item: ${saveTime / 100}ms');
        print('Average read per item: ${readTime / 100}ms');
        
        // Performance expectations (adjust based on actual performance)
        expect(saveTime, lessThan(5000)); // Should save 100 items within 5 seconds
